...
class MesaMamaFama(Strategy):
    params = {
        ...
        # MESA Settings
        "src": Param("ohlc4", options=["close", "open", "high", "low", "hl2", "hlc3", "ohlc4", "hlcc4"],
                     group="MESA Settings"),
    ...
    def init(self, ctx: Context) -> None:
        ...
        # MESA MAMA/FAMA — use configured source (close, hl2, ohlc4, etc.)
        source = bars.get_source(p.src)
        self.mama, self.fama = ta_lib.mesa_mama_fama(
            source, p.fast_limit, p.slow_limit
        )
    ...
    def on_bar(self, bar: BarData, ctx: Context) -> None:
        ...
        # ── Sizing (uses strategy.equity = realized + unrealized) ──
        margin_pct = p.pos_pct / 100.0
        equity = ctx.strategy_equity  # Pine: strategy.equity includes unrealized
        target_notional = equity * margin_pct * p.max_leverage
        raw_qty = target_notional / close if close > 0 else 0
        min_lev_qty = (equity * margin_pct * p.min_leverage) / close if close > 0 else 0
        max_lev_qty = (equity * margin_pct * p.max_leverage) / close if close > 0 else 0
        ...
        # Sizing function for fill-time recalculation (matches TV calc_on_order_fills)
        _margin_pct = margin_pct
        _max_lev = p.max_leverage
        _min_lev = p.min_leverage
        _min_qty = p.min_qty
        def _size_at_fill(fill_price, equity_at_fill):
            tn = equity_at_fill * _margin_pct * _max_lev
            rq = tn / fill_price if fill_price > 0 else 0
            lo = (equity_at_fill * _margin_pct * _min_lev) / fill_price if fill_price > 0 else 0
            hi = (equity_at_fill * _margin_pct * _max_lev) / fill_price if fill_price > 0 else 0
            cq = max(lo, min(rq, hi))
            st = np.floor(cq / _min_qty) * _min_qty
            return max(_min_qty, st)
        ...
        if can_act:
            ...
            if (p.use_swing_logic and is_long and can_reverse_long
                ...
                ctx.close_all()
                ctx.entry("Short", "short", qty=qty_contracts, qty_func=_size_at_fill)
            ...
            # CASE 2: Reversal Short → Long
                ...
                ctx.set_exit_reason("REVERSE")
                ctx.close_all()
                ctx.entry("Long", "long", qty=qty_contracts, qty_func=_size_at_fill)
                reversed_this_bar = True
                self._last_action_bar = i
                self._pending_long = False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  warmup_bars: 0=none (default), -1=auto (70 days for 5m), N=explicit")
        sys.exit(1)
    ...
    # Use DataManager for auto warmup
