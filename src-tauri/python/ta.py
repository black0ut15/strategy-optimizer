...
def mesa_mama_fama(
    ...
    for i in range(6, n):
        ...
        if period[i-1] > 0:
            p1 = max(p1, 0.67 * period[i-1])
            p1 = min(p1, 1.5 * period[i-1])
        ...
        # MAMA and FAMA
        # Use source as fallback (converges faster than Pine's nz=0)
        prev_mama = mama_arr[i-1] if not np.isnan(mama_arr[i-1]) else source[i]
        prev_fama = fama_arr[i-1] if not np.isnan(fama_arr[i-1]) else source[i]
