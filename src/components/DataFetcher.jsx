...
const FUTURES_SYMBOLS = ["ES","NQ","YM","RTY","MES","MNQ","MYM","M2K","CL","NG","GC","SI","ZB","ZN","ZC","ZS","ZW"];
  ...
  const handleFetch = useCallback(async () => {
    ...
    try {
      ...
      if (assetClass === "crypto") {
        ...
        setStatus(`Fetching ${sym} ${cryptoInterval} × ${cryptoDays}d from ${cryptoSource}...`);
        result = await invoke("fetch_crypto", {
          symbol: sym, interval: cryptoInterval, days: cryptoDays, source: cryptoSource,
        });
      } else if (assetClass === "futures") {
        if (!rapidapiKey) { setStatus("Enter your RapidAPI key first"); setFetching(false); return; }
        setStatus(`Fetching ${futuresSymbol} ${futuresInterval} × ${futuresDays}d from InsightSentry...`);
        result = await invoke("fetch_futures", {
          symbol: futuresSymbol, interval: futuresInterval, days: futuresDays, apiKey: rapidapiKey,
        });
      } else {
        ...
        setStatus(`Fetching ${stockSymbol} ${stockInterval} × ${stockDays}d from Alpaca...`);
        result = await invoke("fetch_stocks", {
          symbol: stockSymbol, interval: alpacaInterval, days: stockDays,
          apiKey: alpacaKey, apiSecret: alpacaSecret,
        });
      }

      setStatus(`Fetched ${result.bars.toLocaleString()} bars: ${result.symbol} (${result.first_date} → ${result.last_date})`);

      const info = await invoke("load_data", { path: result.csv_path });
      onDataLoaded(info);
      setStatus(`Loaded ${info.bars.toLocaleString()} bars: ${info.symbol} — ready to optimize`);
    ...
    <div className="max-w-lg mx-auto py-6 px-4">
      ...
      {assetClass === "crypto" && (
        <div className="space-y-1">
          ...
          <div className="text-xs text-zinc-600 pt-2">~ {barEstimate(cryptoInterval, cryptoDays).toLocaleString()} bars · No API key needed</div>
        </div>
      )}
      ...
      {assetClass === "futures" && (
        <div className="space-y-1">
          <Select label="Symbol" value={futuresSymbol} onChange={setFuturesSymbol} options={FUTURES_SYMBOLS} />
          ...
          <div className="text-xs text-zinc-600 pt-2">
            ~ {barEstimate(futuresInterval, futuresDays).toLocaleString()} bars ·
          ...
          <div className="text-xs text-zinc-500 bg-zinc-900 rounded p-2 border border-zinc-800">
            ES, NQ, YM, RTY, MES, MNQ, MYM, M2K, CL, NG, GC, SI, ZB, ZN, ZC, ZS, ZW
          </div>
        </div>
      )}
      ...
      {assetClass === "stocks" && (
        <div className="space-y-1">
          ...
          <div className="text-xs text-zinc-600 pt-2">
            ~ {barEstimate(stockInterval, stockDays).toLocaleString()} bars ·
